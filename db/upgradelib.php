...
/**
 * Upgrade scripts for course format "eTask"
 *
...
/**
 * This method finds all courses in 'etask' format that have actual number of sections
 * different than their 'numsections' course format option.
 *
 * For courses where there are more sections than numsections, we call
 * {@link format_etask_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections. For courses where there are fewer sections
 * than numsections, we call {@link format_etask_upgrade_add_empty_sections()} to add
 * these sections.
 */
function format_etask_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => 'etask', 'format2' => 'etask', 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_etask_upgrade_hide_extra_sections($courseid, $numsections);
    }
    ...
    $DB->delete_records('course_format_options', ['format' => 'etask', 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 */
function format_etask_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
 ...
 * @param int $numsections
 */
function format_etask_upgrade_add_empty_sections($courseid, $numsections) {
    global $DB;
